...
class Weixin extends Component {
    ...
    componentWillMount() {
        ...
        info({state: 1, category_id: WX})
            .then(data => this.setState({total: data.count}))
            .catch(error => message.error(error))
        list({state: 1, limit: 8, offset: 1, category_id: WX})
        ...
        return (
            <Spin spinning={loading}>
                ...
                <div className='image-div-topbar'>
                        <Button type='primary' onClick={() => this.setState({visible: true})}>添加微信</Button>
                        <span className='image-div-topbar-span'>添加互联网上微信资料</span>
                </div>
                <Row>
                {
                    list.map(item => {
...
export default Form.create()(Weixin);